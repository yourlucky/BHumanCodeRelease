  ...
  option
  {
    ...
    {       
      ...
      transition
      {
        ...
        if(theRobotInfo.number == 1&&std::abs(v_angle) < angleToGoalThreshold && std::abs(theFieldBall.positionRelative.y()) < ballYThreshold)
          goto alignBehindBall;
        if(theRobotInfo.number == 2&&std::abs(v_angle_goal) < angleToGoalThreshold && std::abs(theFieldBall.positionRelative.y()) < ballYThreshold)
          goto alignBehindBall;
      }
      ...
      {          
        ...
        ball_X = (_ownPosition.translation.x()-_ballPosition(0));
    ...
    state(alignBehindBall)
    {
      ...
      transition
      {
        if(!theFieldBall.ballWasSeen(ballNotSeenTimeout))
          goto giverole;
       if( theRobotInfo.number == 1&&  std::abs(v_angle) < angleToGoalThresholdPrecise && ballOffsetYRange.isInside(theFieldBall.positionRelative.y()));
          goto kick;
        if( theRobotInfo.number == 2&&  std::abs(v_angle_goal) < angleToGoalThresholdPrecise && ballOffsetYRange.isInside(theFieldBall.positionRelative.y()));
          goto walktotarget;
      }
    ...
    state(walktotarget)
    {
        ...
        action
        {
            theSaySkill("hahahahahahhahahahahahaha");
        }
    }
