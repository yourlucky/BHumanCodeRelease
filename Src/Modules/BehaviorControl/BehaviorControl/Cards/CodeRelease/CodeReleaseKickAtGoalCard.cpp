...
#define EPISODE_LENGTH 10
#define ACTION_LENGTH 50
#define DEBUG_MODE false

int batchStep = 0; // frame index number within batch we are on
int episodeStep =
    -1; // the episode needs to complete one step before we can start logging
int batchIndex = 0;
int actionStep = 0;
...
extern bool RLConfig::train_mode;

auto trajectories =
    json::object{}; // an object to hold the current batch of trajectories
...
unsigned int observationLength =
    (unsigned int)(std::stoi(observationLengthString));
...
void saveTrajectories(int batchIndex) {
  ...
  std::ofstream outputFile(getCurrentDirectory() + "/../trajectories_" +
                           std::to_string(batchIndex) + ".json");
  outputFile << outputString;
  outputFile.close();
  std::ofstream outputFile2(getCurrentDirectory() + "/../complete_" +
                            std::to_string(batchIndex) + ".txt");
...
// worth noting, this is probably not presently windows-compatible due to
// forward slashes in file paths, but I think it should be Mac Compatible
...
{
  std::vector<float> observationVector(4); // observation length is hardcoded
for the time being double x = pose.translation[0]; double y =
pose.translation[1]; double angle = pose.rotation; double sinAngle = sin(angle);
...
std::vector<float> getObservation(GroundTruthRobotPose pose) {
  std::vector<float> observationVector(
      6); // observation length is hardcoded for the time being
  ...
  double target_angle_offset =
      atan2(sin(angle_to_target - angle), cos(angle_to_target - angle));
  ...
  double distance =
      sqrt(std::pow((target_x - x), 2) + std::pow((target_y - y), 2));

  double normalized_distance =
      distance / 4609.77223; // this is the maximum possible euclidiean distance
                             // between two points
  ...
  observationVector[0] = sin(angle);
  observationVector[1] = cos(angle);
...
class CodeReleaseKickAtGoalCard : public CodeReleaseKickAtGoalCardBase {
  ...
  option {
    ...
    initial_state(start) {
      ...
      action {
        ...
        // theStandSkill();
        {
          ...
          SimRobotCore2::Scene *scene =
              (SimRobotCore2::Scene *)RoboCupCtrl::application->resolveObject(
                  "RoboCup", SimRobotCore2::scene);

          // std::cout <<
          // Blackboard::getInstance().exists("GroundTruthWorldState")  <<
          // std::endl;

          if (Blackboard::getInstance().exists(
                  "GroundTruthRobotPose") /* &&
                                             Blackboard::getInstance().exists("GroundTruthWorldState")*/
              && scene != NULL) {

            if ((actionStep % ACTION_LENGTH) == 0) {

              const GroundTruthRobotPose &theGroundTruthRobotPose =
                  static_cast<const GroundTruthRobotPose &>(
                      Blackboard::getInstance()["GroundTruthRobotPose"]);
              // const GroundTruthWorldState& theGroundTruthWorldState =
              // static_cast<const
              // GroundTruthWorldState&>(Blackboard::getInstance()["GroundTruthWorldState"]);
              // const BallState& theBallState = static_cast<const
              // BallState&>(Blackboard::getInstance()["BallState"]);

              // std::cout << theBallState.position << std::endl;

              // const Vector2f ballPosition =
              // theGroundTruthWorldState.balls[0].position.head<2>(); const
              // Vector2f ballVelocity =
              // theGroundTruthWorldState.balls[0].velocity.head<2>();

              // std::cout << ballPosition << std::endl;

              NeuralNetwork::CompilationSettings
                  settings; // not sure if this is necessary but it's unused
                            // since we're not compiling but was required
              ...
              // SimRobotCore2::Scene* scene =
              // (SimRobotCore2::Scene*)RoboCupCtrl::application->resolveObject("RoboCup",
              // SimRobotCore2::scene);
              ...
              */
              ...
              {
              currentObservation =
              getWalkForwardObservation(theGroundTruthRobotPose);
              }
              else if (RLConfig::mode == "ball_targeting")
              {
              currentObservation =
              getBallTargetingObservation(theGroundTruthRobotPose);
              }
              */
              ...
              if (batchStep == 0 &&
                  episodeStep ==
                      -1) // setting up our first trajectories file
                          // descriptor, this will be exchanged on each
              {
                ...
                if (RLConfig::train_mode) {
                  std::remove(
                      (getCurrentDirectory() + "/../shared_policy.h5").c_str());
                  std::remove(
                      (getCurrentDirectory() + "/../action_policy.h5").c_str());
                  std::remove(
                      (getCurrentDirectory() + "/../value_policy.h5").c_str());
                }
              ...
              // std::ifstream
              // metadataFile("/home/john/BHumanCodeRelease/Config/NeuralNets/metadata.json");
              ...
              std::vector<NeuralNetwork::TensorXf> sharedOutputs(
                  sharedModel->getOutputs().size());
              debugPrintString("struct field access");
              std::vector<NeuralNetwork::TensorXf> observationInput(
                  sharedModel->getInputs().size());
              // std::cout << "policy load and input setup complete" <<
              // std::endl; reshaping but not sure why, derived from check.cpp
              const std::vector<NeuralNetwork::TensorLocation> &inputs =
                  sharedModel->getInputs();
              for (std::size_t i = 0; i < observationInput.size(); ++i) {
                observationInput[i].reshape(
                    inputs[i]
                        .layer->nodes[inputs[i].nodeIndex]
                        .outputDimensions[inputs[i].tensorIndex]);
              }
              ...
              */
              ...
              NeuralNetwork::SimpleNN::apply(
                  observationInput, sharedOutputs, *sharedModel,
                  [&settings](
                      const NeuralNetwork::Node &node,
                      const std::vector<const NeuralNetwork::TensorXf *>
                          &inputs,
              ...
              std::vector<NeuralNetwork::TensorXf> valueInput(
                  valueModel->getInputs().size());
              valueInput[0] = latentValue;

              std::vector<NeuralNetwork::TensorXf> valueOutput(
                  valueModel->getOutputs().size());

              NeuralNetwork::SimpleNN::apply(
                  valueInput, valueOutput, *valueModel,
                  [&settings](
                      const NeuralNetwork::Node &node,
                      const std::vector<const NeuralNetwork::TensorXf *>
                          &inputs,
              ...
              std::vector<NeuralNetwork::TensorXf> actionPolicyInput(
                  actionModel->getInputs().size());
              actionPolicyInput[0] = latentAction;

              std::vector<NeuralNetwork::TensorXf> actionPolicyOutput(
                  actionModel->getOutputs().size());

              NeuralNetwork::SimpleNN::apply(
                  actionPolicyInput, actionPolicyOutput, *actionModel,
                  [&settings](
                      const NeuralNetwork::Node &node,
                      const std::vector<const NeuralNetwork::TensorXf *>
                          &inputs,
              ...
              std::vector<float> actionMeanVector =
                  std::vector<float>(); // this vector is created for
                                        // compatibility with floatvectortojson
              ...
              */
              ...
              actionChoice =
                  stats::rmvnorm(actionEigen, covarianceMatrix, true);

              // std::cout << "action: " << std::endl;

              // std::cout << actionChoice << std::endl;

              currentLogProb = stats::dmvnorm(actionChoice, actionEigen,
              ...
              if (episodeStep > -1) {
                ...
                trajectories["observations"].push_back(
                    floatVectToJSON(prevObservation));
                ...
                trajectories["action_means"].push_back(
                    floatVectToJSON(actionMeanVector));
              ...
              if (episodeStep >= EPISODE_LENGTH) {
                // std::cout << stringify(trajectories, json::PrettyPrint) <<
                // std::endl;
                episodeStep = -1;
                ...
                theWalkAtRelativeSpeedSkill(
                    Pose2f(actionChoice(0), actionChoice(1), actionChoice(2)));
              }
    ...
    //     if(!theFieldBall.ballWasSeen(ballNotSeenTimeout))
    //       goto searchForBall;
    //     if(std::abs(angleToGoal) < angleToGoalThreshold &&
    ...
    //     theLookForwardSkill();
    //     theWalkToTargetSkill(Pose2f(walkSpeed, walkSpeed, walkSpeed),
    //     Pose2f(angleToGoal, theFieldBall.positionRelative.x() -
    //     ballAlignOffsetX, theFieldBall.positionRelative.y()));
    //   }
    // }
    ...
    //   transition
    //   {
    //     if(state_time > maxKickWaitTime || (state_time > minKickWaitTime &&
    //     theInWalkKickSkill.isDone()))
    //       goto start;
    //   }
